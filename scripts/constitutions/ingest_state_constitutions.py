...
_WS_INLINE_STATES = {
    ...
    # "az" removed 2026-08-08: replaced by the official-source scrape_az
    # below (azleg.gov, per-section pages). Wikisource had 383 pts / 31
    ...
    # "fl" removed 2026-08-08: replaced by the official-source scrape_fl
    # below (flsenate.gov/Laws/Constitution). Same site/markup convention as
    # FL's own statute scraper; 213 div.Section across 12 div.Article,
    # verified against the source's own structure (see scrape_fl's module
    # note for the live count). Replaces Wikisource purely for provenance
    # (official .gov source vs a third-party mirror), not because the old
    # source was found incomplete.
    # NOT "Constitution_of_Georgia_(2018)" -- that slug is the COUNTRY of
    # Georgia's 2018 constitution (post-Soviet, mentions Abkhazia/South
    # Ossetia), a cross-country contamination bug confirmed live 2026-08-07.
    # The U.S. state's current constitution is at this slug instead.
    "ga": "Constitution_of_the_State_of_Georgia_(1983)",
    # "hi" removed 2026-08-08: replaced by the official-source scrape_hi below
    # (capitol.hawaii.gov HRS reader, sequential-Next per-section crawl).
    # Wikisource had 186 pts / 18 articles, verdict BUG_PRESENT in the C07
    # batch 4 research.
    # "id" removed 2026-08-08: replaced by the official-source scrape_id
    # below (legislature.idaho.gov, 21 article pages -> one page per
    # section, reusing scrapeID.py's own proxy + div.pgbrk pattern).
    # Wikisource had 248 pts / 21 articles, verdict CLEAN (no split bug,
    # just a third-party mirror).
    # "il" removed 2026-08-08: replaced by the official-source scrape_il
    # below (ilga.gov/commission/lrb/con{1-14}.htm). Same "SECTION N. TITLE
    # ... (Source: ...)" convention already handled by scrapeIL.py's statute
    # parser; 140 sections across all 14 real articles, verified live.
    # "ks" removed 2026-08-08: replaced by the official-source scrape_ks
    # below (sos.ks.gov, the Secretary of State's own publications site, 18
    # per-document pages). Wikisource had 245 pts / 16 articles, verdict
    # BUG_PRESENT.
    # "la" removed 2026-08-08: replaced by the official-source scrape_la
    # below (senate.la.gov PDF, C07 batch 6). Wikisource had its own
    # multipart-subpage workaround (scrape_louisiana_wikisource_multipart,
    # left in the file unused, see STATE_SCRAPERS assembly below for why
    # its override line was removed too).
    # "ma" removed 2026-08-08: replaced by the official-source scrape_ma
    # below (malegislature.gov/Laws/Constitution). Reuses the SHAPE of the
    # Wikisource-era _parse_ma/_parse_ma_part2 fix (Part > Chapter > Section
    # > Article) against the new domain's real semantic h2/h3/h4 tags -- see
    # scrape_ma's own module note for why the old regexes don't carry over.
    # "md" removed 2026-08-08: replaced by the official-source scrape_md
    # below (mgaleg.maryland.gov, same GetNext/GetPrevious walk API and
    # div#StatuteText template MD's own statute scraper already uses; the
    # constitution article codes ('c0'-'c20') are already sitting unused in
    # that scraper's own Articles dropdown, explicitly filtered out there).
    # "mn" removed 2026-08-08: replaced by the official-source scrape_mn below
    # (revisor.mn.gov/constitution/). Wikisource had 139 pts across the same
    # 14 articles; the official page's clean div.article/div.section DOM is
    # now walked directly instead.
    # "mo" removed 2026-08-08: replaced by the official-source scrape_mo
    # below (revisor.mo.gov, the same ASP.NET `constit=y` site MO's statute
    # scraper uses; 402 sections across all 14 articles, incl. Article XIV
    ...
    # 318 pts / 20 articles, verdict CLEAN.
    "nh": "New_Hampshire_Constitution_(2019)",
    # "nj" removed 2026-08-08: replaced by the official-source scrape_nj
    # below (njleg.state.nj.us/constitution). Reuses _parse_nj (Article >
    # optional SECTION sub-level > numbered paragraph) unchanged against the
    # new domain -- the body shape is identical to the retired Wikisource
    # page, confirmed live 2026-08-08.
    # "nm" removed 2026-08-07: replaced by the official-source scrape_nm
    # below (sos.nm.gov PDF). Wikisource only had 3 of the real 23 articles.
    # "nv" removed 2026-08-07: replaced by the official-source scrape_nv
    # below (leg.state.nv.us). Wikisource's own page admitted "This work is
    # incomplete... source document not known" and only ever carried the
    # Preamble + Article I (2 of the real ~232 sections); the official page
    # (reachable through the US proxy, contra the earlier Cloudflare-403
    # assumption) has the complete, currently-maintained document.
    # "ny" removed 2026-08-08: replaced by the official-source scrape_ny
    # below (legislation.nysenate.gov OpenLeg API, `GET /laws/CNS` -- the
    # SAME API NY's own statute corpus already uses officially). Wikisource
    # had 302 pts / 20 articles, verdict NEEDS_REVIEW; this supersedes that
    # review with the real, currently-maintained official source rather than
    # re-verifying the old partial fix.
    # "oh" removed 2026-08-08: replaced by the official-source scrape_oh below
    # (codes.ohio.gov/ohio-constitution, same CMS as OH's own statute
    # scraper). Wikisource had 229 pts / 18 articles, verdict CLEAN.
    # "ok" removed 2026-08-08: replaced by the official-source scrape_ok
    # below (oklegislature.gov RTF, C07 batch 6).
    # "or" removed 2026-08-08: replaced by the official-source scrape_or
    # below (oregonlegislature.gov/bills_laws/Pages/OrConst.aspx). This is a
    # SECOND replacement of "or"'s entry -- it previously pointed at
    # scrape_oregon_wikisource_subpages (see that function's own module note
    # for why Wikisource needed a subpages walk in the first place), which
    # is now itself retired along with every other Wikisource path for this
    # state.
    "sc": "Constitution_of_South_Carolina",
    "sd": "Constitution_of_South_Dakota",
    # "tn" removed 2026-08-08: replaced by the official-source scrape_tn
    # below (publications.tnsosfiles.com PDF, C07 batch 6).
    # "va" removed 2026-08-08: replaced by the official-source scrape_va below
    # (law.lis.virginia.gov, same LIS platform VA's own statute scraper
    # already reaches, one page per section). Wikisource had 149 pts / 12
    # articles, verdict CLEAN.
    # "vt" removed 2026-08-08: replaced by the official-source scrape_vt
    # below (legislature.vermont.gov). Reuses the existing
    # _VT_ARTICLE_RE/_VT_SECTION_RE pair (see _ART_SPLIT_OVERRIDES/
    # _SECTION_SPLIT_OVERRIDES below) unchanged against the new domain.
    # "wa" removed 2026-08-07: replaced by the official-source scrape_wa
    # below (leg.wa.gov PDF). Wikisource's own page was itself an
    # incomplete transcription (12 of 32 real articles were bare stubs).
    # "wi" removed 2026-08-08: replaced by the official-source scrape_wi
    # below (docs.legis.wisconsin.gov PDF, two-column layout). Wikisource
    # had 188 pts / 14 articles, verdict CLEAN.
    # "wv" removed 2026-08-07: replaced by the official-source scrape_wv
    # below (home.wvlegislature.gov). Wikisource only ever transcribed
    # Articles I-V (48 of the real 204 sections); no regex fix could recover
    # the missing Articles VI-XIV since the page simply never had them.
    # "wy" removed 2026-08-08: replaced by the official-source scrape_wy
    # below (wyoleg.gov, "Title 97" codified PDF, C07 batch 6).
}
...
def scrape_wikisource_inline(state: str, slug: str, r2) -> list[Section]:
    ...
    try:
        ...
        _DROPS.fetch_failed(f"{state} wikisource", e)
        return out
    ...
    soup = BeautifulSoup(html, "html.parser")
    # NOTE: stripping "[edit]" mw-editsection boilerplate here was tried and
    # reverted 2026-08-07 -- it looks like pure noise cleanup but it collapses
    # a "\n" that some states' section-header regexes depend on (VA dropped
    # from 127 to 116 real sections with a new false zero-section article
    # once the span was removed, confirmed by a direct before/after diff
    # against the live page). The `[edit]` text is left in raw_text as a
    # minor, known cosmetic artifact rather than risk silently merging or
    # dropping sections. If this is worth doing, it needs the SAME
    # regression rigor as a section-split-regex change (test against every
    # affected state's real section count, not just VA), not a one-line
    # decompose() added as a side effect of an unrelated cluster fix.
    ...
    # A minority of states use a document structure the generic Article/
    # Section split below cannot represent (no "Article"/"Section" keyword
    # at all, or a nesting level the generic 2-level walk doesn't have -- see
    # docs/us-corpus/handoffs/constitutions/HANDOFF_C00_index.md §5). Those
    # states get a small dedicated parser instead; everything else falls
    # through to the generic path completely unchanged.
    custom_parser = _CUSTOM_PARSERS.get(state)
    if custom_parser is not None:
        out = custom_parser(state, body_text, url, r2_html_url, r2)
        if not out:
            _DROPS.unit_empty(f"{state} wikisource custom parser (0 sections)")
        print(f"  [{state}] custom parser: {len(out)} sections from {url}")
        return out
    ...
    body_prep = _BODY_PREP_OVERRIDES.get(state)
    if body_prep is not None:
        body_text = body_prep(body_text)

    art_split_fn = _ART_SPLIT_FN_OVERRIDES.get(state)
    if art_split_fn is not None:
        # A regex swap can't express CT's two-pass (spelled-ordinal main
        # articles, then Roman-numeral amendments) split -- see
        # `_ct_split_articles`. This bypasses the regex-split path below
        # entirely for the handful of states that need it.
        art_iter = art_split_fn(body_text)
    else:
        art_split_re = _ART_SPLIT_OVERRIDES.get(state)
        if art_split_re is None:
            art_split_re = re.compile(
                r"\n(?:ARTICLE|Article)\s+([IVXLC\d]+(?:[\.\-][IVXLC\d]+)?(?:[A-Z])?)[\.\:]?(?:[ \t][^\n]*)?\n"
            )
        art_parts = art_split_re.split(body_text)
    ...
    # A real constitution never legitimately repeats the same article number
    # for two different articles. A repeat is always a TOC/nav artifact
    # emitting short heading fragments ahead of the real numbered content --
    # confirmed live 2026-08-07 for CO, whose two-column Wikisource TOC
    # produces 15 near-empty, MISLABELED article fragments (off-by-one: a
    # short body for "Article XIII" that is actually the TOC's rendering of
    # the *next* entry, "Article XIV: Counties") ahead of the real,
    # complete, correctly-labelled body. Keep only the longest-bodied
    # occurrence of each article_id; a strict no-op for every state whose
    # article_ids are already unique (verified: this cannot change output
    # for VA/NC/WI, the reference known-good states, since none of them
    # produce a duplicate article_id).
    if len(art_iter) != len({a for a, _ in art_iter}):
        best: dict[str, tuple[str, str]] = {}
        for a_id, a_body in art_iter:
            if a_id not in best or len(a_body) > len(best[a_id][1]):
                best[a_id] = (a_id, a_body)
        art_iter = list(best.values())

    sec_split_re = _SECTION_SPLIT_OVERRIDES.get(state)
    sec_split_fallback_re = _SECTION_SPLIT_FALLBACK_OVERRIDES.get(state)
    for art_id, art_body in art_iter:
        # Split on Section markers within this article
        if sec_split_re is not None:
            sec_parts = sec_split_re.split(art_body)
        else:
            sec_parts = re.split(
                r"\n(?:SECTION|Section|Sec\.)\s+(\d+(?:\.\d+)?[A-Za-z]?)\.?\s*", art_body
            )
        if len(sec_parts) <= 1 and sec_split_fallback_re is not None:
            # The state's primary convention found nothing in THIS specific
            # article -- try its registered fallback convention (see
            # _SECTION_SPLIT_FALLBACK_OVERRIDES) before giving up to the
            # whole-article section_number="0" blob below. Gated on the
            # primary split already having failed, so this can never
            # regress an article that already splits correctly under the
            # state's normal pattern.
            sec_parts = sec_split_fallback_re.split(art_body)
        if len(sec_parts) <= 1:
            # Fall back: emit the whole article as one SECTION record.
            # (Previously capped at 5000 chars — that produced audit-flagged
            # mid-word truncations on WI/NJ/KS/WV/SD/AK/MD articles, so the cap
            # was dropped and the full body passed through. That in turn made
            # these the longest bodies in the corpus, up to 172,512 chars, and
            # embed_and_upsert truncates its embedding input at 30,000. The
            # section is now split by to_chunk_records, so the whole article is
            # reachable by both dense and sparse search.)
        ...
        for sec_num, sec_text in sec_iter:
            ...
            out.append(sec)
    if not out:
        # Fetched but the article/section walk (and the whole-article fallback)
        # yielded nothing: a silent whole-state drop without this guard.
        _DROPS.unit_empty(f"{state} wikisource (0 sections)")
...
_OR_INDEX_SLUG = "Oregon_Constitution"
...
def _wikisource_page_body(html: str):
    """Candidate-div selection identical in spirit to scrape_wikisource_inline
    (pick the longest of mw-parser-output / mw-content-ltr / mw-content-text),
    plus stripping `<span class="mw-editsection">[edit]</span>` markers so
    they cannot leak into section text -- OR's per-article subpages carry one
    of these per Section heading, far more than the single-page states this
    helper does NOT touch (it is only ever called from the OR-specific
    scraper below, never from scrape_wikisource_inline)."""
    soup = BeautifulSoup(html, "html.parser")
...
def scrape_oregon_wikisource_subpages(r2) -> list[Section]:
    """Oregon: Wikisource's main page is an article-LINK index, not inline
    text (see the module note above). Walk it, fetch every Article_* subpage,
    and split sections using the same "Section N" convention
    scrape_wikisource_inline already relies on for every other state."""
    state = "or"
    index_url = f"https://en.wikisource.org/wiki/{_OR_INDEX_SLUG}"
    out: list[Section] = []
    try:
        index_html = fetch_text(index_url)
    except Exception as e:
        _DROPS.fetch_failed(f"{state} wikisource index", e)
        return out

    r2_html_key = f"state_constitutions/{state}/source/wikisource_main.html"
    put_if_changed(r2, r2_html_key, index_html.encode("utf-8"), "text/html; charset=utf-8")

    index_body = _wikisource_page_body(index_html)
    seen_hrefs: set[str] = set()
    article_links: list[tuple[str, str]] = []  # (article_id, subpage_url)
    for a in index_body.find_all("a", href=True):
        href = a["href"]
        article_id = _or_article_id_from_href(href)
        if article_id is None or href in seen_hrefs:
            continue
        seen_hrefs.add(href)
        article_links.append((article_id, f"https://en.wikisource.org{href}"))

    if not article_links:
        _DROPS.unit_empty(f"{state} wikisource index (0 article links found)")
        return out

    for art_id, sub_url in article_links:
        try:
            sub_html = fetch_text(sub_url)
        except Exception as e:
            _DROPS.fetch_failed(f"{state} wikisource {art_id}", e)
            continue

        r2_sub_html_key = f"state_constitutions/{state}/source/wikisource_article_{art_id}.html"
        put_if_changed(r2, r2_sub_html_key, sub_html.encode("utf-8"), "text/html; charset=utf-8")
        r2_sub_html_url = public_url(r2_sub_html_key)

        body = _wikisource_page_body(sub_html)
        body_text = body.get_text("\n", strip=True)
        body_text = _or_isolate_article_segment(body_text, art_id)

        # Article XVIII (Schedule) headers its sections as "Article XVIII,
        # section N" instead of the plain "Section N." form every other OR
        # subpage uses. This alternation is a strict superset (adds the
        # "Article <roman>, section N" form alongside the existing plain
        # form) so it cannot regress any other article. Verified live
        # 2026-08-07 against en.wikisource.org/wiki/Oregon_Constitution/Article_XVIII:
        # recovers 11 real sections from what was one section_number="0" blob.
        sec_parts = re.split(
            r"\n(?:(?:SECTION|Section|Sec\.)\s+|Article\s+[IVXLC]+,\s*section\s+)"
            r"(\d+(?:\.\d+)?[A-Za-z]?)\.?\s*",
            body_text,
        )
        if len(sec_parts) <= 1:
            ...
            if text:
                sec = Section(
                    ...
                    article_id=art_id,
                    section_number="0",
                    section_title=f"OR Const., Article {art_id}",
                    raw_text=text,
                    source_url=sub_url,
                    r2_html_url=r2_sub_html_url,
                )
                out.append(sec)
            else:
                _DROPS.unit_empty(f"{state} wikisource {art_id} (0 text)")
            continue
        ...
        for sec_num, sec_text in sec_iter:
            ...
            sec = Section(
                ...
                article_id=art_id,
                section_number=sec_num,
                section_title=f"OR Const., Article {art_id}, Section {sec_num}",
                raw_text=text,
                source_url=sub_url,
                r2_html_url=r2_sub_html_url,
            )
            r2_txt_key = f"state_constitutions/{state}/sections/SCONST_OR_A{art_id}_S{sec_num}.txt"
            put_if_changed(
                r2, r2_txt_key, sec.raw_text.encode("utf-8"), "text/plain; charset=utf-8"
            )
            sec.r2_txt_url = public_url(r2_txt_key)
    ...
    if not out:
        _DROPS.unit_empty(f"{state} wikisource subpages (0 sections total)")
    print(
        f"  [{state}] wikisource subpages: {len(out)} sections across {len(article_links)} articles"
    )
    return out
...
def scrape_louisiana_wikisource_multipart(r2) -> list[Section]:
    """Louisiana: real article text is spread across the main Wikisource page
    (Articles I-III) plus 6 Part_N subpages (Articles IV-XIV). See the module
    note above for why a plain single-page fetch mis-splits this state."""
    state = "la"
    out: list[Section] = []
    slugs = [_LA_MAIN_SLUG, *_LA_PART_SLUGS]
    art_split_re = re.compile(
        r"\n(?:ARTICLE|Article)\s+([IVXLC\d]+(?:[\.\-][IVXLC\d]+)?(?:[A-Z])?)[\.\:]?(?:[ \t][^\n]*)?\n"
    )
    ...
    n_articles_total = 0
    for slug in slugs:
        page_url = f"https://en.wikisource.org/wiki/{slug}"
        try:
            html = fetch_text(page_url)
        except Exception as e:
            _DROPS.fetch_failed(f"{state} wikisource {slug}", e)
            continue

        page_name = slug.rsplit("/", 1)[-1] if "/" in slug else "main"
        r2_html_key = f"state_constitutions/{state}/source/wikisource_{page_name}.html"
        put_if_changed(r2, r2_html_key, html.encode("utf-8"), "text/html; charset=utf-8")
        r2_html_url = public_url(r2_html_key)

        body = _wikisource_page_body(html)
        body_text = _la_trim_nav(body.get_text("\n", strip=True))

        art_parts = art_split_re.split(body_text)
        if len(art_parts) <= 1:
            _DROPS.unit_empty(f"{state} wikisource {slug} (no Article splits)")
            continue
        art_iter = [(art_parts[i], art_parts[i + 1]) for i in range(1, len(art_parts) - 1, 2)]
        n_articles_total += len(art_iter)
        ...
        for art_id, art_body in art_iter:
            ...
            if len(sec_parts) <= 1:
                ...
                if text:
                    sec = Section(
                        ...
                        article_id=art_id,
                        section_number="0",
                        section_title=f"LA Const., Article {art_id}",
                        raw_text=text,
                        source_url=page_url,
                        r2_html_url=r2_html_url,
                    )
            ...
            for sec_num, sec_text in sec_iter:
                ...
                sec = Section(
                    ...
                    article_id=art_id,
                    section_number=sec_num,
                    section_title=f"LA Const., Article {art_id}, Section {sec_num}",
                    raw_text=text,
                    source_url=page_url,
                    r2_html_url=r2_html_url,
                )
                r2_txt_key = (
                    f"state_constitutions/{state}/sections/SCONST_LA_A{art_id}_S{sec_num}.txt"
                )
                put_if_changed(
                    r2, r2_txt_key, sec.raw_text.encode("utf-8"), "text/plain; charset=utf-8"
                )
    ...
    if not out:
        _DROPS.unit_empty(f"{state} wikisource multipart (0 sections total)")
    print(
        f"  [{state}] wikisource multipart: {len(out)} sections across "
        f"{n_articles_total} article-page-entries from {len(slugs)} pages"
    )
    return out

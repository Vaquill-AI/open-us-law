...
act_id scheme (a ``code=`` level first, matching the legacy scrape):
...
14:95.1.1); the legacy scrape wrote dashes (14:95-1). So the RS act_ids
reproduce the base sections but not the dotted sub-sections byte-for-byte, and
the article codes cannot reproduce the a commercial mirror roman-numeral title at all. That

...
reconcilable): ``STATE_PA_T{title}_C{chapter}_S{section}`` where
  ...
  - ``chapter`` is DERIVED from the section number by the same heuristic the
    original a commercial mirror scraper used (leading digits of the section minus the
...
The one difference from the legacy act_ids is the section separator: the
a commercial mirror slug encoded the decimal as a hyphen (``3132-1``) whereas the official
